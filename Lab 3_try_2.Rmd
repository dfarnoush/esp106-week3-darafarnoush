...
airpolution <- read.csv("airpollution.csv")
GDP <- read.csv("gdppercapiandgini.csv")

colnames(airpollution) <- c("country", "code", "year", "ambient", "fuel", "ozone", "pollution")
colnames(GDP) <- c("country", "code", "year", "population", "continent", "gini", "percapita")
colnames(GDP)
```
...
```{r question2}
unique(airpollution$country)

bhutan <- airpollution[airpollution$country=="Bhutan",]
View(outdoorb)
outdoorb <- bhutan$fuel + bhutan$ozone
Bhutan <- cbind(bhutan, outdoorb)

wales <- airpollution[airpollution$country=="Wales",]
View(wales)
outdoorw <- wales$fuel + wales$ozone
Wales <- cbind(wales, outdoorw)

plot(Bhutan$year, Bhutan$ambient, "n", xlab = "Year", ylab = "Deaths/Year", main = "Deaths per Year in Bhutan and Wales", ylim = c(0, 150))
  lines(Bhutan$year, Bhutan$ambient, "l", col = "yellow4", lty = "solid")
  lines(Bhutan$year, Bhutan$outdoorb, "l", col = "yellow4", lty = "dashed")
  lines(Wales$year, Wales$ambient, "l", col = "green", lty = "solid")
  lines(Wales$year, Wales$outdoorw, "l", col = "green", lty = "dashed")
  legend("topright", legend = c("Bhutan Indoor Air Pollution", "Bhutan Outdoor Air Pollution", "Wales Indoor Air Pollution", "Wales Outdoor Air Pollution"), col = c("yellow4", "yellow4", "green", "green"), lty = c("solid", "dashed", "solid", "dashed"), bg = "gray95")
...
```{r question3}
airpollutionbyGDP <- merge(x=airpollution, y=GDP, by.x = c("country", "year"), by.y =c("country", "year"))
```
...
```{r question4}
par(mfrow=c(1,2))
GDPlog <- log(airpollutionbyGDP$percapita)
plot(GDPlog, airpollutionbyGDP$ambient, pch = 20, xlab = "GDP (log)", ylab = "Outdoor Air Pollution", main = "Indoor Pollution vs GDP")
plot(GDPlog, airpollutionbyGDP$fuel + airpollutionbyGDP$ozone, pch = 20, xlab = "GDP (log)", ylab = "Indoor Air Pollution", main = "Outdoor Pollution vs GDP")
```
